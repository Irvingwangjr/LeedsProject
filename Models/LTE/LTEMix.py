...
from Models.Modules.FusionModule import LTEBilinearFusion, LTEMixFusion


class LTEMix(BaseModel):
    ...
    def loadFusionModule(self):
        return LTEMixFusion(HyperParam.AutoIntFeatureNum, HyperParam.AutoIntLayerDims,
    ...
    def loadMatchingModule(self):
        HyperParam.AutoIntWithFieldMatchMlpDims[0] = int(
            (HyperParam.AutoIntFeatureNum * HyperParam.AutoIntFeatureNum) * HyperParam.AutoIntFeatureDim)
    ...
    #     output = torch.cat(fusionFeature, dim=1)
    #     return output
